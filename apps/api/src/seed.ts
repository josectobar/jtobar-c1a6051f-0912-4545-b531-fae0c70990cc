...
async function seed() {
  ...
  try {
    ...
    // --- Users (upsert by email — always sync role, orgId, password) ---
    const seedUsers = [
    ...
    for (const u of seedUsers) {
      ...
      if (existing) {
        await userRepo.update(existing.id, { role: u.role, orgId: root.id, password: hashedPassword });
        savedUsers.push({ ...existing, role: u.role, orgId: root.id });
      } else {
